...
var RemoteTabViewer = require('synctabs').RemoteTabViewer;
...
Cu.import("resource:///modules/NewTabUtils.jsm", this);
...
if (isFirefox) {
  ...
  var workerFunction = function (worker) {
    worker.port.on('tpemit', function (data) {
      if (data === 'initialized') {
        RemoteTabViewer.init(function (links) {
          worker.port.emit('tabs', links);
        });

        NewTabUtils.links.populateCache(function () {
          worker.port.emit('sites', NewTabUtils.links.getLinks());
        });
